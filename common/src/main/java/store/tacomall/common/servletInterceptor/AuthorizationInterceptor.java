...
public class AuthorizationInterceptor implements HandlerInterceptor {
    ...
    public boolean preHandle(HttpServletRequest request, HttpServletResponse response, Object handler)
        ...
        if (StringUtil.isBlank(appAccessToken)) {
            ExceptionUtil.throwUnauthorizedException("ServiceAccessToken is require");
        }
        ...
        if (!authFeignClient.verifyServiceAccessToken(appAccessToken).getStatus()) {
            ExceptionUtil.throwUnauthorizedException("ServiceAccessToken verify fail");
        }
